...
Usage:
  srbiz [--project-dir DIR] [--volumes] [--no-system-prune] [--builder-prune] [--clear-logo-cache] [--logs]
...
Options:
  ...
  --clear-logo-cache    Wipe ./app/.cache/biznes-logo-cache on the host
...
if [[ "${CLEAR_LOGO_CACHE}" -eq 1 ]]; then
  CACHE_DIR="${PROJECT_DIR}/app/.cache/biznes-logo-cache"
